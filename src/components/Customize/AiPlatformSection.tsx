...
import { evaluateQualityGate, loadAiLayers, mergeAiLayers, parseWorkflowTasks, qualityReviewStoreKey, saveAiLayer, validateExtensionManifest, workflowRolePrompt } from '../../services/aiPlatform';
...
const emptyReview: WorkspaceReviewState = { reviewedKeys: [], tests: [] };
...
export function AiPlatformSection({ category, changes }: Props) {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      if (savedReview && typeof savedReview === 'object') setReview({ ...emptyReview, ...savedReview });
  ...
  if (category === 'quality') return <section>
    ...
    <div className="grid gap-4 lg:grid-cols-2"><div className="rounded-lg border border-slate-200 bg-white dark:border-[#303030] dark:bg-[#202020]"><div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-[#303030]"><h3 className="text-sm">{t('customize.diffReview')}</h3><button onClick={() => void persistReview({ ...review, reviewedKeys: changes.filter((change) => change.kind !== 'deleted').map(changeKey) })} className="rounded border border-slate-300 px-2 py-1 text-xs dark:border-[#444]">{t('customize.markAllReviewed')}</button></div>{changes.length ? changes.map((change) => <label key={changeKey(change)} className="flex items-center gap-3 border-b border-slate-100 px-4 py-3 text-xs last:border-b-0 dark:border-[#303030]"><input type="checkbox" disabled={change.kind === 'deleted'} checked={review.reviewedKeys.includes(changeKey(change))} onChange={(event) => void persistReview({ ...review, reviewedKeys: event.target.checked ? [...new Set([...review.reviewedKeys, changeKey(change)])] : review.reviewedKeys.filter((key) => key !== changeKey(change)) })} /><span className="min-w-0 flex-1 truncate">{change.name}{change.language ? ` · ${change.language}` : ''}</span><span className="text-slate-500">{change.kind}</span></label>) : <div className="p-8 text-center text-xs text-slate-500">{t('customize.workspaceNoChanges')}</div>}</div>
